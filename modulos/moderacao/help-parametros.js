...
function parametrosPT(P) {
  ...
  return {
    ...
    tts: [
      ...
      { nome: "resgatar", valor: "[#call-auxiliar]", desc: "quando `destravar` não resolve: entra numa call auxiliar do mesmo servidor, move-se para a call presa pelo PATCH do Stoat (que emite token sem conferir o registro) e conecta de verdade *(ManageMessages)*" },
...
function parametrosEN(P) {
  ...
  return {
    ...
    tts: [
      ...
      { nome: "resgatar", valor: "[#helper-call]", desc: "when `destravar` doesn't cut it: joins a helper call in the same server, moves itself into the stuck call through Stoat's PATCH (which issues a token without checking the record) and connects for real *(ManageMessages)*" },
